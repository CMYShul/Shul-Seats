...
## 2025-05-16 - [Selection State Visibility]
**Learning:** For forms with many numeric inputs (like seat selection), providing immediate visual feedback at the row level when a quantity is entered helps users quickly verify their "cart" at a glance. Auto-selecting text on focus further reduces friction for quantity adjustments.
**Action:** Use a `.selected` class to highlight active rows and implement `input.select()` on focus for numeric fields.
